...
interface UserStats {
  guessTheElo: {
    ...
    ratingDistribution: {     // Success rate for different rating ranges
  ...
  guessTheEval: {
    ...
    bestStreak: number;
    totalPoints: number;
    positionTypes: {         // Success rate for different position types
    ...
    <div className="min-h-screen bg-[#0f1011] text-white">
      <div className="max-w-[1200px] mx-auto p-6">
        ...
        <div className="flex items-center mb-8">
          ...
          <h1 className="text-2xl font-bold ml-4">Profile Dashboard</h1>
        </div>
          ...
          <div className="col-span-12 lg:col-span-6">
            <Tabs defaultValue="overall" className="w-full">
                ...
                <Card className="bg-[#1a1b1e] border-[#2a2b2d]">
                  <CardHeader>
                    ...
                    <CardDescription className="text-gray-400">
                      Your combined performance across all modes
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="grid grid-cols-2 gap-4">
                      <StatCard
                        title="Total Games"
                        value={userData.stats.overall.totalGamesPlayed}
                        icon={<Activity className="h-4 w-4 text-blue-500" />}
                      />
                      <StatCard
                        title="Rank"
                        value={userData.stats.overall.rank}
                        icon={<Crown className="h-4 w-4 text-yellow-500" />}
                      />
                      <StatCard
                        title="Member Since"
                        value={new Date(userData.stats.overall.joinedDate).toLocaleDateString()}
                        icon={<Calendar className="h-4 w-4 text-green-500" />}
                      />
                      <StatCard
                        title="Last Active"
                        value={new Date(userData.stats.overall.lastPlayed).toLocaleDateString()}
              ...
              <TabsContent value="guessElo">
                <Card className="bg-[#1a1b1e] border-[#2a2b2d]">
                  <CardHeader>
                    <CardTitle>Guess the Elo Stats</CardTitle>
                    <CardDescription className="text-gray-400">
                  ...
                  <CardContent>
                    <div className="space-y-6">
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <StatCard
                          title="Games Played"
                          value={userData.stats.guessTheElo.gamesPlayed}
                          icon={<ChessKnight className="h-4 w-4 text-blue-500" />}
                        />
                        <StatCard
                          title="Accuracy"
                        ...
                        <StatCard
                          title="Best Streak"
                          value={userData.stats.guessTheElo.bestStreak}
                          icon={<Flame className="h-4 w-4 text-orange-500" />}
                        />
                        <StatCard
                          title="Total Points"
                          value={userData.stats.guessTheElo.totalPoints}
                          icon={<Star className="h-4 w-4 text-yellow-500" />}
                        />
                      </div>

                      <div>
                        <h4 className="text-sm font-medium mb-3">Rating Range Performance</h4>
                        <div className="space-y-2">
                          <ProgressStat
                            label="Beginner (<1200)"
                            value={userData.stats.guessTheElo.ratingDistribution.beginner}
                          />
                          <ProgressStat
                            label="Intermediate (1200-1600)"
                            value={userData.stats.guessTheElo.ratingDistribution.intermediate}
                          />
                          <ProgressStat
                            label="Advanced (1600-2000)"
                            value={userData.stats.guessTheElo.ratingDistribution.advanced}
                          />
                          <ProgressStat
                            label="Expert (>2000)"
                            value={userData.stats.guessTheElo.ratingDistribution.expert}
                          />
                        </div>
                      </div>
                    </div>
                    ...
                    <HoverCard key={index}>
                      ...
                      <HoverCardContent className="bg-[#1a1b1e] border-[#2a2b2d]">
                        <div className="text-sm">
                          Achievement details and progress could go here
                        </div>
                      </HoverCardContent>
                    </HoverCard>
                  ))}
